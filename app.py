...
def main():
    ...
    if st.session_state.submitted:
        ...
        st.info(
            f"Percentage: {(st.session_state.score / st.session_state.total * 100):.1f}%"
        )
    ...
    with st.form("quiz_form"):
        ...
        permanent_address = st.text_input(
            "Permanent Address *", placeholder="Enter your permanent address"
        )
        facility_address = st.text_input(
            "Facility Address *", placeholder="Enter your facility address"
        )
        ...
        if submitted:
            ...
            try:
                ...
                save_result_details(
                    RESULTS_DETAILS_FILE, user_info, answers, questions, timestamp
                )
